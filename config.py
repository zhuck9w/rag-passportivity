...
# 2026-07-10: поднят 0.54 → 0.55 — после ресинка Португалии оффтоп «как
# оформить отпуск» стал цеплять её фрагмент с 0.542; новый зазор 0.542→0.559.
MIN_SIMILARITY = 0.55

# Спасение пустой выдачи (только intent=knowledge, не-survey ветки).
# Лексический спасатель: если после порога MIN_SIMILARITY фрагментов ноль,
# редкие термины вопроса (аббревиатуры вроде FIU, NHR, визы D7) ищутся по
# точному вхождению в текст чанков. Потолок — чтобы точечный термин не
# притащил в промпт простыню из всех упоминаний.
RESCUE_MAX_FRAGMENTS = 4
# Подсказка «почти попал»: если и спасатель пуст, но лучший хит векторного
# поиска лёг в [NEAR_MISS_MIN, MIN_SIMILARITY) — вопрос скорее всего «про
# наше», просто сформулирован иначе (реальный случай: 0.475 у вопроса про
# возвратность взноса Вануату). Тогда подсказываем страну/программу/раздел
# лучшего хита. Ниже 0.45 — оффтоп («борщ», «корпоратив»), подсказка вредна.
NEAR_MISS_MIN = 0.45

# Обзорный режим: вопрос «про многие страны сразу» без конкретной страны
# (например, «какие опции ВНЖ есть в ЕС?»). Глобальный топ-K тут не работает:
# похожести таких вопросов ложатся плато 0.48-0.53 ниже MIN_SIMILARITY,
# поэтому берём лучшие фрагменты с КАЖДОЙ страны с мягким порогом.
SURVEY_MIN_SIMILARITY = 0.45   # мягкий порог обзорного режима (плато обзорных вопросов 0.48-0.53)
SURVEY_PER_COUNTRY = 2         # лучших фрагментов с каждой страны
SURVEY_MAX_FRAGMENTS = 40      # 40 = 31 лучший фрагмент каждой страны + ~9 слотов глубины; покрытие прежде всего
